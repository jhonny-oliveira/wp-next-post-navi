...
if($options['comingsoon'] != "no"){
		...
		function handle_wp_next_post_navi_options()
		{
			...
			if (isset($_POST['submitted']))
			{
				...
				$settings['position']       = $_POST['position']    ?? "bottom" ;
				$settings['comingsoon']     = $_POST['comingsoon']  ?? "yes" ;
				$settings['is_custom']      = $_POST['is_custom']   ?? "yes" ;
				$settings['corner']         = $_POST['corner']      ?? "0" ;
				$settings['shadow']         = $_POST['shadow']      ?? "yes" ;
				$settings['related']        = $_POST['related']     ?? "yes" ;
				$settings['style']          = $_POST['style_css']   ?? "" ;
				$settings['is_button']      = isset($_POST['is_button'])        ? "1" : "0" ;
				$settings['custom_pre']     = $_POST['custom_pre']  ??   "" ;
				$settings['custom_next']    = $_POST['custom_next'] ??  "" ;
				$settings['custom_next_sub']        = $_POST['custom_next_sub']         ??  "" ;
				$settings['custom_pre_color']       = $_POST['custom_pre_color']        ??  "" ;
				$settings['custom_next_color']      = $_POST['custom_next_color']       ??  "" ;
				$settings['custom_next_sub_color']  = $_POST['custom_next_sub_color']   ??  "" ;
				$settings['custom_pre_text_color']  = $_POST['custom_pre_text_color']   ??  "" ;
				$settings['custom_next_text_color'] = $_POST['custom_next_text_color']  ??  "" ;
				$settings['custom_next_sub_text_color'] = $_POST['custom_next_sub_text_color']  ??  "" ;
				$settings['navi_img']       = isset($_POST['navi_img']) ? "1" : "0" ;
				$settings['pre_img_link']   = $_POST['pre_img_link']    ??  "" ;
				$settings['next_img_link']  = $_POST['next_img_link']   ??  "" ;
	...
	return $links;
		...
		function WP_Next_Post_Navi_install()
		{
			...
			$options = array(
				...
				'is_reversed'       => $options["is_reversed"]      ?? '0',
				'position'          => $options["position"]         ?? 'bottom',
				'comingsoon'        => $options["comingsoon"]       ?? 'yes',
				'is_custom'         => $options["is_custom"]        ?? 'yes',
				'corner'            => isset($options["corner"])    ?   $options["is_custom"] : 'corner',
				'shadow'            => $options["shadow"]           ??  'yes',
				'related'           => $options["related"]          ??  'yes',
				'nav_within_cat'    => $options["nav_within_cat"]   ??  '0',
				'style'             => $options["style"]            ??  'text-decoration: none;
                                                                         font:bold 16px sans-serif, arial;
                                                                         color: #666;',
				'is_button'         => $options["is_button"]        ??  '1',
				'custom_pre'        => $options["custom_pre"]       ??  'Previous Post',
				'custom_next'       => $options["custom_next"]      ??  'Next Post',
				'custom_next_sub'   => $options["custom_next_sub"]  ??  'Next Post Coming Soon...',
				'custom_pre_color'  => $options["custom_pre_color"] ??  '#4CAF50',
				'custom_next_color' => $options["custom_next_color"]??  '#008CBA',
				'custom_next_sub_color'         => $options["custom_next_sub_color"]        ??  '#008CBA',
				'custom_pre_text_color'         => $options["custom_pre_text_color"]        ??  '',
				'custom_next_text_color'        => $options["custom_next_text_color"]       ??  '',
				'custom_next_sub_text_color'    => $options["custom_next_sub_text_color"]   ??  '',
				'navi_img'          => $options["navi_img"]         ??  '0',
				'pre_img_link'      => $options["pre_img_link"]     ??  '',
				'next_img_link'     => $options["next_img_link"]    ??  '',
